...
// visualization_msgs::Marker createMarkerMsg(std::vector<kuka_cv::Color> & colors) {
...
//         color.r = colors[i].r/255;
//         color.g = colors[i].g/255;
//         color.b = colors[i].b/255;
//         color.a = 1.0;
...
int main(int argc, char ** argv)
{
    ...
    switch(part) {
        case 1: {
            ...
            for (size_t i = 0; i < palette.colors.size(); ++i) {
                ...
                ROS_INFO_STREAM("[LTP] Color: [" << palette.colors[i].r
                                          << ", " << palette.colors[i].g
                                          << ", " << palette.colors[i].b << "]");
    ...
    //                 ROS_INFO_STREAM("\t Successful");
    //             }
    //             palette = paletteInfo.response.Colors;
    //             ROS_WARN_STREAM("[LTP] Receive Colors array size = 0");
